...
pub const Batch = struct {
    allocator: Allocator,
    input_ids: *tensor.IntTensor,
    attention_mask: *tensor.ByteTensor,
    labels: *tensor.IntTensor,

    pub fn deinit(self: *Batch) void {
        self.input_ids.deinit();
        self.allocator.destroy(self.input_ids);
        self.attention_mask.deinit();
        self.allocator.destroy(self.attention_mask);
        self.labels.deinit();
        self.allocator.destroy(self.labels);
    }
};
...
pub const TextDataset = struct {
    ...
    pub fn getBatch(self: *TextDataset, allocator: Allocator, batch_size: usize) !Batch {
        ...
        const input_ids = try allocator.create(tensor.IntTensor);
        input_ids.* = try tensor.IntTensor.init(allocator, &[_]usize{ batch_size, max_seq_len });
        input_ids.fill(0);
        errdefer { input_ids.deinit(); allocator.destroy(input_ids); }

        const attention_mask = try allocator.create(tensor.ByteTensor);
        attention_mask.* = try tensor.ByteTensor.init(allocator, &[_]usize{ batch_size, max_seq_len });
        attention_mask.fill(0);
        errdefer { attention_mask.deinit(); allocator.destroy(attention_mask); }

        const labels = try allocator.create(tensor.IntTensor);
        labels.* = try tensor.IntTensor.init(allocator, &[_]usize{ batch_size, max_seq_len });
        labels.fill(-100);
        errdefer { labels.deinit(); allocator.destroy(labels); }
        ...
        while (i < batch_size) : (i += 1) {
            ...
            while (j < seq_length) : (j += 1) {
                ...
                input_ids.data[idx] = sample.input_ids.items[j];
                attention_mask.data[idx] = if (sample.attention_mask.items[j]) 1 else 0;
                labels.data[idx] = sample.input_ids.items[j];
            }
        }
