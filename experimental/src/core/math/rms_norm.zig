...
pub fn rmsNorm(input: []const f32, weight: []const f32, output: []f32, eps: f32) void {
    // Use comptime conditions instead of std.debug.assert to avoid release mode issues
    if (builtin.mode == .Debug) {
        if (input.len != weight.len or input.len != output.len) {
            std.debug.panic("RMS norm dimension mismatch: input={}, weight={}, output={}", .{ input.len, weight.len, output.len });
        }
    }
